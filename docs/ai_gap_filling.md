...
The research pass fills missing results for exactly the nineteen Avg IQ benchmarks
...
- llm-stats model detail pages, matched against all nineteen configured components.
...
- Vals embedded benchmark data: MMLU-Pro, AIME 2025, LiveCodeBench v6,
  SWE-bench Verified, GPQA Diamond, and Terminal-Bench 2.1. Select the exact task:
  AIME 2025 excludes the combined 2024–2025 headline score; other adapters use
  overall accuracy, never a subject/difficulty subset.
...
`data/historical_benchmark_evidence.json` and fill current gaps before paid research.
...
nineteen benchmark columns are saved. Avg IQ uses their fixed weights and a 100%
