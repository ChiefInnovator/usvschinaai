...
SYSTEM_PROMPT = (
    ...
    "hashtags (6 to 8, no spaces, mix broad and specific). Do not include URLs, website domains, "
    "or any mention of a link or comment."
)
...
def render_caption(caption: Dict[str, Any]) -> str:
    lines = [caption["hook"], ""] + [f"• {b}" for b in caption["bullets"]] + ["", caption["question"], "",
             " ".join(t if t.startswith("#") else f"#{t}" for t in caption["hashtags"])]
    return "\n".join(lines)
